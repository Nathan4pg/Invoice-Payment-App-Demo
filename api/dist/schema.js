...
for (let i = 0; i < 100; i++) {
    ...
    const shippingCompany = shippingCompanies[Math.floor(Math.random() * shippingCompanies.length)];
...
exports.resolvers = {
    Query: {
        invoices: (_, { first, after, sortBy, sortDirection, }) => {
            ...
            if (after) {
                const cursorIndex = sortedInvoices.findIndex((invoice) => invoice.id === after);
                if (cursorIndex !== -1) {
